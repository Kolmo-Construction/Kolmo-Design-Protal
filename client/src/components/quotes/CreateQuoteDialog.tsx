...
export function CreateQuoteDialog({ open, onOpenChange }: CreateQuoteDialogProps) {
  ...
  const createQuoteMutation = useMutation({
    mutationFn: async (data: CreateQuoteForm) => {
      ...
      const quote = await apiRequest("/api/quotes", "POST", quoteData);
      
      // Create line items if any exist
      if (data.lineItems.length > 0) {
        for (const lineItem of data.lineItems) {
          if (lineItem.description && lineItem.quantity > 0) {
            await apiRequest(`/api/quotes/${quote.id}/line-items`, "POST", {
              ...lineItem,
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea 
                                placeholder="Describe the milestone for payment..."
                                {...field} 
                              />
                            </FormControl>
                            <FormMessage />
